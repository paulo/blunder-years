...
void drawGrid(float comprimento, float largura){
    float *vertexB=NULL;
    ...
    for(linha=0;linha < th-1; linha++) {
    	...
    	for(coluna=0;coluna<tw;coluna++){
    		glVertex3f(altura_aux,h(coluna,linha),largura_aux);
    		glVertex3f(altura_aux - altura, h(coluna, linha+1), largura_aux);
